...
const Header = () => {
  ...
  return (
    <div className='sticky top-0 z-50 border-b border-gray-800 backdrop-blur-md'>
      <div className='flex items-center justify-between py-4 container mx-auto max-w-screen-xl px-4'>
        ...
        <div className='hidden md:flex items-center gap-6'>
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className='text-gray-800 hover:text-gray-600 font-bold transition-colors uppercase text-sm lg:text-base'
            >
              {link.label}
            </Link>
          ))}
