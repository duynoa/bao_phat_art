...
const CardProject = ({ project }: ProjectProps) => {
  return (
    <Link href={`/du-an/${project.slug || project._id}`} className="relative flex flex-col md:flex-row w-full h-auto md:h-[200px] border border-gray-300 rounded-lg overflow-hidden">
      <div className="w-[52%] p-4 flex flex-col justify-between">
        <div className="flex flex-col gap-2">
          <h2 className="text-title1 uppercase text-gray-900 line-clamp-1">{project.name}</h2>
          <div className="text-base text-gray-900 line-clamp-3">
            <div dangerouslySetInnerHTML={{ __html: removeSpace(project.summary || "") }} />
          </div>
        </div>
        <div className="flex w-full gap-3 text-gray-900 truncate">
          ...
          <div className="flex items-center gap-1">
            ...
            <p className="text-sm truncate">{project.completionYear}</p>
          </div>
          <div className="flex items-center gap-1 truncate">
            <CropFreeIcon sx={{ fontSize: "16px" }}/>
            <p className="text-sm truncate">{project.type}</p>
          </div>
        </div>
      </div>
      <div className="w-[48%] flex-shrink-0 aspect-[3/2] md:aspect-auto md:h-full">
        <Image
